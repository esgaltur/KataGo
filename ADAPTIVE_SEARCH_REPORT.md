...
## 4. TengenGo Backend Integration

To complete the end-to-end feature lifecycle, the adaptive search toggle was elevated to the Rust backend (`crates/bot/src/lib.rs` and `gtp.rs`).
...
- **Production Enablement**: `Master` and `Superhuman` tiers now pass `adaptive_search: true` dynamically.
- **Analysis Integration**: The interactive game review layer (`QueryOptions::for_analysis`) was upgraded to default to `adaptive_search: true`, providing the user with maximum-precision engine evaluations for complex board variations.
